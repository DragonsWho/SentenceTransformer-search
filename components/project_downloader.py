...
logging.basicConfig(
    ...
    level=logging.WARNING,                                                          # будет глючить - поменяй на DEBUG и смотри файл logs/project_downloader.log
...
def detect_encoding(content):
    ...
    """
    Detects the encoding of the given content using chardet.
    """
...
def is_valid_url(url, base_domain):
    """
    Validates the URL to ensure it is well-formed and belongs to the base domain.
    Excludes URLs containing backslashes or other suspicious characters.
    """
    parsed = urlparse(url)
...
def is_local_resource(src, base_url):
    """
    Determines if the resource is local relative to the base URL.
    """
    if src.startswith('http://') or src.startswith('https://'):
        return urlparse(src).netloc == urlparse(base_url).netloc
    if src.startswith('//'):
        return urlparse(base_url).netloc == src.split('/')[2]
    return True  # Relative URLs are considered local
...
def get_game_name(url):
    """
    Extracts the domain and game name from the URL.
    """
...
def enumerate_project_resources(data, directories=['images', 'music', 'videos', 'fonts', 'css', 'js', 'audio', 'assets']):
    """
    Recursively extracts file paths from specified directories in JSON data.
    """
...
def create_session():
    """Создает оптимизированную сессию requests"""
    ...
    adapter = HTTPAdapter(
        ...
        pool_connections=100,    # количество соединений в пуле
        pool_maxsize=100,       # максимальное количество соединений
        pool_block=False        # неблокирующий режим пула
    )
    
    # Применяем адаптер для HTTP и HTTPS
...
def download_file(url, path, session, base_domain, retries=3, delay=5, request_delay=0.1):
    ...
    # Проверяем существование файла с использованием HEAD запроса
    if path.exists():
        try:
            ...
            if head.status_code == 304:  # Not Modified
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            with session.get(url, stream=True, timeout=15) as response:
                ...
                if is_text_file:
                    # Для текстовых файлов читаем всё содержимое
                    content = response.content
                    encoding = detect_encoding(content)
                    text = content.decode(encoding, errors='replace')
                    path.write_text(text, encoding='utf-8')
                else:
                    # Для бинарных файлов используем потоковую запись
...
def parse_html_for_resources(html_content, base_url, base_domain):
    ...
    # Парсим base_url для получения схемы и домена
    parsed_base = urlparse(base_url)
    
    # Убедимся, что base_url заканчивается на слеш
        ...
        if src:
            ...
            if is_local_resource(src, base_url):
                if not src.startswith('data:'):
                    # Если src начинается с /, добавляем к домену
                    if src.startswith('/'):
    ...
    # Additionally, search for .js files within embedded scripts
...
def parse_css_for_resources(css_content, base_url, base_domain):
    """
    Parses CSS content to extract all local resource URLs.
...
def handle_resource(full_url, session, base_path, base_url_path, base_domain):
    ...
    """
    Обрабатывает и скачивает один ресурс.
    """
    ...
    # Правильная очистка base_url_path_clean без ведущего слеша
    ...
    if path.startswith(base_url_path_clean):
        ...
        logging.debug(f"Base URL Path Clean: '{base_url_path_clean}', Path: '{path}', Relative Path: '{relative_path}'")
    else:
        relative_path = path
        logging.debug(f"Base URL Path Clean: '{base_url_path_clean}', Path does not start with it. Using full path: '{relative_path}'")

    # Конструируем локальный путь
    file_path = os.path.join(base_path, relative_path)
    logging.debug(f"Saving resource to: '{file_path}'")

    # Проверяем, что URL валиден (проверка уже прошла в `parse_html_for_resources`, но сохраняем для надёжности)
    ...
    # Если CSS, парсим его на дополнительные ресурсы
...
def crawl_and_download(url, base_path, session=None, max_workers=5):
    """
    Crawls the given URL and downloads all associated resources, saving them to base_path.
    """
    if session is None:
    ...
    try:
        ...
        # Обработка контента с правильной кодировкой
        ...
        # Скачиваем project.json с оптимизированными настройками
        ...
        # Параллельное скачивание с оптимизированным пулом потоков
